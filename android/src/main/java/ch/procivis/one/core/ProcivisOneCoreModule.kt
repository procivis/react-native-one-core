...
class ProcivisOneCoreModule(reactContext: ReactApplicationContext) :
    ...
    fun holderSubmitProofV2(
        ...
        asyncCall(promise, scope) {
            ...
            for (entry in credentials.entryIterator) {
                ...
                submission[entry.key] = credentialList
            }
            getCore().holderSubmitProofV2(
                interactionId,
                submission
            )
            return@asyncCall null
    ...
    @ReactMethod
    fun createCredentialSchema(request: ReadableMap, promise: Promise) {
        asyncCall(promise, scope) {
            return@asyncCall getCore().createCredentialSchema(construct(request))
        }
    }
