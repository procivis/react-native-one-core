...
class ProcivisOneCoreModule: RCTEventEmitter {
  ...
  func createIdentifier(
    ...
    reject: @escaping RCTPromiseRejectBlock) {
      asyncCall(resolve, reject) {
        return try await self.getCore().createIdentifier(request: deserializeIdentifierRequest(identifierRequest: identifierRequest));
      }
    }
  ...
  func getIdentifiers(
    ...
    reject: @escaping RCTPromiseRejectBlock) {
      asyncCall(resolve, reject) {
        let listQuery = try deserializeIdentifierListQuery(query);
        let result = try await self.getCore().listIdentifiers(query: listQuery);
        return serialize(identifierList: result)
      }
    }
